        ...
        <main className="flex-grow p-4 overflow-y-auto min-h-[20rem]">
          {isLoading ? (
            ...
            <div className="relative">
              <img src={images[currentIndex]} alt={`预览 ${topic} 第 ${currentIndex + 1} 部分`} className="w-full h-auto rounded-lg border" />
              {images.length > 1 && (
                <>
                  <button onClick={goToPrevious} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white rounded-full p-2 shadow-md transition-all" aria-label="上一张">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                  </button>
                  <button onClick={goToNext} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white rounded-full p-2 shadow-md transition-all" aria-label="下一张">
